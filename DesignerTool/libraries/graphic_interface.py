...
#    Button(rootWindowHandler, text="Computations from curves", command=computationsFromCurves).pack()
    ...
    rootWindowHandler.title("Speaker impedance capturer/designer tool - I. D. Simon, 2015.")
...
def displayCurvesCapturing(filename):
    ...
    fig = plot.gcf()
    fig.canvas.set_window_title("Capturing speaker impedance curve from image file: '" + onlyFilename + "'.")

    # Asks for maximum axes values
    global max_frequency
    global max_impedance
    d = InputDialog(rootWindowHandler, "Max. value on X-axis (Hz):")
    rootWindowHandler.wait_window(d.top)
    max_frequency = outputDataVar
    d = InputDialog(rootWindowHandler, "Max. value on Y-axis (Ohms):")
    rootWindowHandler.wait_window(d.top)
    max_impedance = outputDataVar
...
def onCapturePlotWindowClosedEvent(event):
    ...
    print 'Data:', curvesData

    # Ask for filename and save
    _outputFile = tkFileDialog.asksaveasfilename(title="Save file to ...", defaultextension=".zee")
    if _outputFile != "":
        saveDataToFile(_outputFile, curvesData, curvesCorners, max_frequency, max_impedance)
...
def onCapturePlotClickedEvent(event):
    ...
    updatedPlotText = "Step 2 - Create curve with mouse clicks.\nLeft: new point. Right: delete last point.\nClose window to finish."
    ...
    # Add/remove point
        ...
        # Right click, delete last point (if any)
        if event.button == 3:
            if len(curvesData) > 0:
...
def updatePlots(updatedPlotText=""):
    ...
    if len(curvesCorners) == 4:
        ...
        if len(curvesCorners) > 0:
            [xCoords, yCoords] = xyCoordinatesToTwoVectors(curvesCorners)
            xCoords.extend([xCoords[0]])
            yCoords.extend([yCoords[0]])
            plot.plot(xCoords, yCoords, 'x', color='blue')
            xCoords.remove([xCoords[-1]])
            yCoords.remove([yCoords[-1]])

    # Plot all data available
    if len(curvesData) > 0:
        [xCoords, yCoords] = xyCoordinatesToTwoVectors(curvesData)
        plot.plot(xCoords, yCoords, 'x-', color='red')
...
def saveDataToFile(outputFilename, curvesData, curvesCorners, maxVoltage, maxCurrent):
    '''
    ...
    _newCurve = []
    for i in range(len(curvesData)):
        # Add new V/I point
        _v = (curvesData[i][0] - _positionOrigin[0]) / (_xLim - _positionOrigin[0]) * maxVoltage
        _i = (curvesData[i][1] - _positionOrigin[1]) / (_yLim - _positionOrigin[1]) * maxCurrent
        _newCurve.extend([[_v, _i]])
    _allCurves.extend([_newCurve])

    # Save all data to a file
    if len(outputFilename) < 4:
        _filenameToSave = outputFilename + '.zee'
    else:
        if outputFilename[-4:] != '.zee':
            _filenameToSave = outputFilename + '.zee'
        else:
